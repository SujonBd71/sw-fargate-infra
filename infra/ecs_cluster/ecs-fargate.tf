...
resource "aws_alb_listener" "sw-ecs_alb_https_listener" {
  load_balancer_arn = aws_alb.sw-ecs_cluster_alb.arn
  port              = 443
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS-1-2-2017-01"
  certificate_arn   = aws_acm_certificate.sw-ecs_domain_certificate.arn

  default_action {
    type             = "forward"
    target_group_arn = aws_alb_target_group.sw-ecs_default_target_group.arn
  }

  depends_on = [aws_alb_target_group.sw-ecs_default_target_group]
}

resource "aws_alb_target_group" "sw-ecs_default_target_group" {
